...
from src.previous_research.ofdm_simulation_si import simulation as previous_simulation
...
if __name__ == '__main__':
    ...
    previous_y_ratio = np.zeros((params.graph_x_num, params.trials))
    ...
    for trials_index in tqdm(range(params.trials)):
        ...
        for graph_x_index, SNR in enumerate(graph_x_array):
            ...
            y = previous_nn_model.pred_system_model.y
            y_hat = previous_nn_model.y_hat.reshape(y.shape, order='F')
            diff = y - y_hat
            # diff = y
            diff_abs = np.abs(diff)**2
            diff_sum = np.sum(diff_abs)
            y_sum = np.sum(np.abs(y)**2)
            y_ratio = diff_sum / y_sum
            # y_ratio_db = 10 * np.log10(y_ratio)

            previous_y_ratio[graph_x_index][trials_index] = y_ratio
    ...
    n_sum = params.trials
    errors_sum = np.sum(previous_y_ratio, axis=1)
    y_ratio = errors_sum / n_sum
    ber_ax.plot(graph_x_array, y_ratio, color="k", marker='d', linestyle='--', label="Conventional")
