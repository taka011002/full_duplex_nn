...
if __name__ == '__main__':
    ...
    np.place(bers, bers == 0, None)
    ax.plot(snrs_db, bers, color='k', marker='x', linestyle='-', label=r"$w/o canceller$", ms=12)

    errors_sum = np.sum(result.previous_errors, axis=1)
    bers = errors_sum / previous_n_sum
    np.place(bers, bers == 0, None)
    ax.plot(snrs_db, bers, color='k', marker='*', linestyle='-', label=r"$Previous$", ms=12)
    ...
    bers = errors_sum / n_sum
    np.place(bers, bers == 0, None)
    ax.plot(snrs_db, bers, color='k', marker='^', linestyle='-', label=r"$Proposal(antennae = 1)$", ms=12)
    ...
    bers = errors_sum / n_sum
    np.place(bers, bers == 0, None)
    ax.plot(snrs_db, bers, color='k', marker='o', linestyle='-', label=r"$Proposal(antennae = 2)$", ms=12)
    ...
    bers = errors_sum / n_sum
    np.place(bers, bers == 0, None)
    ax.plot(snrs_db, bers, color='k', marker='d', linestyle='-', label=r"$Proposal(antennae = 3)$", ms=12)
